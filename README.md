...
The backend deduplicates entries with an `ON CONFLICT` clause on `(repository_id, iid, username, spent_at, time_spent_seconds)` and reports usernames that do not exist in the `intern` table so the UI can inform operators. Each persisted row now carries both the raw seconds and a precomputed `time_spent_hours` value (rounded to six decimals) so payroll exports can operate directly on hours without recomputing them on every fetch.
